...
A service in a manifest YAML must exist in `services.proto`.

**Example error:**

```bash
services in manifests but not defined in proto: [InvalidServiceName1, InvalidSerciveName2]
```

**Note:** It is perfectly valid for services to exist in `services.proto` but
...
**Example:** `manifests/scenario1.yaml`
...
This creates a scenario named `"scenario1"` with 2 services `"HostService"` and `"InstanceService"`.
