...
func TestProvider_Patch(t *testing.T) {
	log.Info().Msgf("Begin Provider Patch tests")
	ctx, cancel := context.WithTimeout(context.Background(), testTimeout)
	defer cancel()
	...
	// Create a Provider
	provider := CreateProvider(t, ctx, apiClient, utils.Provider1Request)
	assert.Equal(t, utils.ProviderName1, provider.JSON200.Name)

	// Modify fields for patching
	newName := utils.ProviderName1 + "-updated"
	newApiEndpoint := "https://updated-endpoint.com"
	patchRequest := api.ProviderResource{
		Name:        newName,
		ApiEndpoint: newApiEndpoint,
	}

	// Perform the Patch operation
	updatedProvider, err := apiClient.ProviderServicePatchProviderWithResponse(
		ctx,
		*provider.JSON200.ResourceId,
		nil,
		patchRequest,
		AddJWTtoTheHeader, AddProjectIDtoTheHeader,
	)
	require.NoError(t, err)
	assert.Equal(t, http.StatusOK, updatedProvider.StatusCode())
	assert.Equal(t, newName, updatedProvider.JSON200.Name)
	assert.Equal(t, newApiEndpoint, updatedProvider.JSON200.ApiEndpoint)

	// Verify the changes with a Get operation
	getProvider, err := apiClient.ProviderServiceGetProviderWithResponse(
		ctx,
		*provider.JSON200.ResourceId,
		AddJWTtoTheHeader, AddProjectIDtoTheHeader,
	)
	require.NoError(t, err)
	assert.Equal(t, http.StatusOK, getProvider.StatusCode())
	assert.Equal(t, newName, getProvider.JSON200.Name)
	assert.Equal(t, newApiEndpoint, getProvider.JSON200.ApiEndpoint)

	log.Info().Msgf("End Provider Patch tests")
}
