...
// Defines values for OSUpdatePolicyUpdatePolicy.
const (
	UPDATEPOLICYLATEST OSUpdatePolicyUpdatePolicy = "UPDATE_POLICY_LATEST"
	UPDATEPOLICYTARGET OSUpdatePolicyUpdatePolicy = "UPDATE_POLICY_TARGET"
)
...
type OSUpdatePolicy struct {
	...
	// InstallPackages Freeform text, OS-dependent. A list of package names, one per line (newline separated). Must not contain version information. Applies only to Mutable OSes.
	InstallPackages *string `json:"installPackages,omitempty"`

	// KernelCommand The OS resource's kernel Command Line Options. Applies only to Mutable OSes.
	...
	// UpdateSources The list of OS resource update sources. Should be in 'DEB822 Source Format' for Debian style OSs. Applies only to Mutable OSes.
	UpdateSources *[]string `json:"updateSources,omitempty"`
}

// OSUpdatePolicyUpdatePolicy Update Policy for the OS update. This field is used to determine the update policy for the OS update. UPDATE_POLICY_LATEST: - for mutable: unsupported - for immutable: latest version of the OS Resource UPDATE_POLICY_TARGET: - for mutable: apply the install_packages, update_sources, kernel_command - for immutable: install the version referenced by target_os
type OSUpdatePolicyUpdatePolicy string
