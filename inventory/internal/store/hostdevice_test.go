...
func Test_UpdateHostdevice(t *testing.T) {
	...
	testcases := map[string]struct {
		...
		"UpdateHostdevice1": {
			...
			fieldMask: &fieldmaskpb.FieldMask{
				Paths: []string{hostdeviceresource.FieldHostname},
			},
			valid: true,
		},
		"UpdateHostdevice2": {
			...
			fieldMask: &fieldmaskpb.FieldMask{
				Paths: []string{
					...
					hostdeviceresource.FieldHostname,
				},
			},
			valid: true,
		},
...
func Test_FilterHostdevices(t *testing.T) {
	...
	testcases := map[string]struct {
		...
		"FilterHostname": {
			in: &inv_v1.ResourceFilter{
				Filter: fmt.Sprintf(`%s = %q`, hostdeviceresource.FieldHostname, expHostdevice2.GetHostname()),
			},
			resources: []*computev1.HostdeviceResource{expHostdevice2},
			valid:     true,
		},
		"FilterHostnameEmpty": {
			in: &inv_v1.ResourceFilter{
				Filter: fmt.Sprintf(`%s = ""`, hostdeviceresource.FieldHostname),
			},
