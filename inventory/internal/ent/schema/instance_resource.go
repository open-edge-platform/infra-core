...
func (InstanceResource) Edges() []ent.Edge {
	return []ent.Edge{edge.To("host", HostResource.Type).Unique(), edge.To("desired_os", OperatingSystemResource.Type).Required().Unique(), edge.To("current_os", OperatingSystemResource.Type).Unique(), edge.To("os", OperatingSystemResource.Type).Unique(), edge.From("workload_members", WorkloadMember.Type).Ref("instance"), edge.To("provider", ProviderResource.Type).Unique(), edge.To("localaccount", LocalAccountResource.Type).Unique()}
}
