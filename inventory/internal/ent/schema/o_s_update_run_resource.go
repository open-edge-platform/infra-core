...
func (OSUpdateRunResource) Fields() []ent.Field {
	return []ent.Field{field.String("resource_id").Unique(), field.String("name").Optional().Immutable(), field.String("description").Optional().Immutable(), field.Enum("status_indicator").Values("STATUS_INDICATION_UNSPECIFIED", "STATUS_INDICATION_ERROR", "STATUS_INDICATION_IN_PROGRESS", "STATUS_INDICATION_IDLE"), field.String("status").Optional(), field.String("status_details").Optional(), field.String("status_timestamp").SchemaType(map[string]string{"postgres": "TIMESTAMP"}), field.String("start_time").Immutable().SchemaType(map[string]string{"postgres": "TIMESTAMP"}), field.String("end_time").Optional().SchemaType(map[string]string{"postgres": "TIMESTAMP"}), field.String("tenant_id").Immutable(), field.String("created_at").Immutable().SchemaType(map[string]string{"postgres": "TIMESTAMP"}), field.String("updated_at").SchemaType(map[string]string{"postgres": "TIMESTAMP"})}
}
